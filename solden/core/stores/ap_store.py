...
def _ensure_memory_payload_for_audit_event(
    ...
    *,
    ...
    """Attach operational memory to every canonical audit row.

    Explicit memory events keep their caller-provided shape. Legacy audit rows
    are promoted into memory events here so the product does not depend on
    every caller remembering a second write.
    """
    from solden.services.audit_memory import ensure_memory_payload_for_audit_event

    return ensure_memory_payload_for_audit_event(
        payload,
        box_type=box_type,
        box_id=box_id,
...
class APStore:
        ...
        with self.connect() as conn:
            ...
            if prev_state is not None and new_state is not None:
                ...
                if isinstance(decision_context, dict) and decision_context:
                    ...
                    audit_payload["decision_context"] = auto_ctx
                audit_payload = _ensure_memory_payload_for_audit_event(
                    {
                        "event_type": "state_transition",
                        "from_state": prev_state,
                        "to_state": kwargs["state"],
                        "actor_type": actor_type,
                        "actor_id": actor_id,
                        "source": audit_source,
    ...
    def append_audit_event(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        payload_json = _ensure_memory_payload_for_audit_event(
            payload,
    ...
    def set_ap_item_owner_atomic(
        ...
        *,
        ...
        """
        owner_audit_payload = dict(audit_payload or {})
        owner_audit_payload.setdefault(
            "owner",
            {
                "id": owner_id,
                "email": owner_email,
        ...
        owner_audit_payload = _ensure_memory_payload_for_audit_event(
            {
                "event_type": "owner_changed",
                "actor_type": actor_type,
                "actor_id": actor_id,
                "source": "box_owner.apply_resolved_owner",
                "correlation_id": correlation_id,
                "decision_reason": decision_reason,
                "organization_id": organization_id,
                "ts": now,
        ...
        try:
            with self.connect() as conn:
                ...
                cur.execute(insert_sql, (
                    ...
                    actor_type,
                    actor_id,
                    json.dumps(owner_audit_payload or {}),
                    json.dumps({}),
                    idempotency_key,
