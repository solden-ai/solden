...
Solden is the missing layer: a persistent, attributable home for every workflow instance, with an agent that advances each one where it can and escalates to a human only on the exception. We call each instance a **Box**. The Box holds state, timeline, exceptions, and outcome. In the shipped product, the AP invoice is the first Box. Vendor onboarding, commission clawbacks, purchase orders, and bank match are expansion paths, not current customer-facing surfaces. The Box is the product. Gmail, Slack, the ERP, and the workspace recovery surface are how the Box is rendered.

Finance teams interact with Solden inside the tools they already use. AP work happens in Gmail — the sidebar renders the Box, the custom route shows the queue, the exceptions surface in the inbox. Approvals happen in Slack. Posted records live in the ERP. The workspace shell exists for setup, admin, exception recovery, and audit; it is not the daily workflow desktop and it should not become a second place where operators reconstruct work from memory.
