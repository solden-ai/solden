...
|---|---|---|---|---|---|---|---|
| Callback duplication | Duplicate callback/action is idempotent and auditable | Duplicate result callbacks are idempotent in browser-agent channel path | `tests/test_browser_agent_layer.py::test_duplicate_result_is_idempotent` | PASS (channel-path) | qa-eng | L03 | Slack/Teams live callback duplication still pending staging evidence |
...
| Posting failure after approval | Item moves to failure path with clear recovery options | API-failure and fallback request behavior covered in automated tests | `tests/test_erp_api_first.py::test_post_bill_api_first_requests_browser_fallback_on_api_failure` | PASS | qa-eng | L03 | Staging confirmation pending |
| Browser fallback failure | Failure remains auditable and operator-visible in `failed_post` | Automated tests confirm failure remains in failed state with audit event | `tests/test_browser_agent_layer.py::test_browser_fallback_complete_failure_keeps_failed_post_and_audits` | PASS | qa-eng | L03 | Staging confirmation pending |
| Confidence gate block before posting | Low-confidence invoice requires manual intervention/escalation | Decision-layer tests cover low-confidence escalation behavior | `tests/test_ap_decision.py::test_fallback_low_confidence` | PASS (decision-layer) | qa-eng | L09 | Needs full staging scenario with operator flow |
| Restart during retry/fallback | Durable retry jobs persist and resume after restart | Durable retry and post-process restart coverage passes in automated suite | `tests/test_agent_orchestrator_durable_retry.py::test_durable_retry_job_survives_restart_and_posts_to_erp`, `tests/test_agent_orchestrator_durable_retry.py::test_durable_post_process_job_survives_restart_and_completes` | PASS | platform-eng | L04 | Staging restart drill still required for pilot evidence |
