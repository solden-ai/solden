...
| 2026-06-03 | Workspace workflow nav labels name box types | WORKFLOWS items should name the workflow / box type the operator is entering, not the generic data object. The AP surface is labeled Accounts Payable; individual rows remain records. `/accounts-payable` is the route; `/records` is not a product route. |
